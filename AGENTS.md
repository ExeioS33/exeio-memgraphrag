...
- LightRAG-*shaped* API, not a drop-in replacement: the same four surfaces (documents / query / graph / Ollama `/api`) with 46 operations against LightRAG's ~47, and differing response shapes on several of them. A LightRAG client can be pointed at this server for the common query/ingest calls, but expect to adapt; treat parity as partial and unverified per route.
