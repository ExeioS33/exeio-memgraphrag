...
def test_ssl_verify_default_true(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("memgraphrag.utils.http_ssl.find_configured_ca_path", lambda: None)
    reset_ssl_verify_cache()
    assert ssl_verify() is True


@pytest.mark.offline
def test_ssl_context_with_custom_ca(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
