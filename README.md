...
**Agent mode needs a tool-capable model.** Whether the loop performs a *second*
search is a property of the model: `meta-llama/Llama-3.3-70B-Instruct-Turbo` and
`moonshotai/Kimi-K3` re-search with a reformulated query, while neither `gpt-oss`
variant ever does — even given unlimited time. On a two-part question whose halves
live in different documents, `agent` answered both halves from 2 documents where
`ppr` answered one and said so, at 17.8 s against 6.4 s. Pin the models you trust
with `AGENT_TOOL_MODELS`, or expect the mode to behave like `ppr` plus a round trip.
